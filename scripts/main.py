...
def testParameters(hidden_layers: int, neurons: int, max_epochs: int, learning_rate: float, setpoint: float, isJordan: bool):
    ...
    for i in range(neuralController.ncConfig.max_epochs-1):
        u[i+1] = neuralController.run(y[i])
        y[i+1] = pt2.step(u[i+1])
        #t[i] = i
        if (i%1000) == 0:
            print(f"Epoch: {i} Plant output: {y[i]} u: {u[i]} Error: {neuralController.ncConfig.setpoint - y[i]}")
...
def main():
    ...
    data = testParameters(2, 8, 5000, 0.01, 1.0, True)
    t = data[:,0]
    y = data[:,1]
    u = data[:,2]
    plt.figure()
